...
<div class="container-fluid text-center">    
  <div class="row content">
	...
	$daftar = mysqli_query($koneksi, "SELECT * FROM data_antrian");
	echo "<table id='t01' border='1' style='width:100%'>";
	echo "<tr><th>No. Antrian</th><th>No. ID</th><th>Tanggal Daftar</th><th>Tanggal Pemeriksaan</th></tr>";
	
	while($row = mysqli_fetch_row($daftar)) {
		echo '<tr><td>'.$row[0].'</td><td>'.$row[1].'</td><td>'.$row[2].'</td><td>'.$row[3].'</td>
			<td><a href="./edit.php?id="'.$row[0].'" />Edit</a> | 
				<a href="./proseshapus.php?id='.$row[0].'" />Hapus</a>
			</td></tr>';
	}
	echo "</table>";
	?>
